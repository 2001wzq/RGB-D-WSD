...
from Code.lib.FuseModule import FuseModule0
from Code.lib.FuseModule import FuseModule1
from Code.lib.FuseModule import GNNFuse
import timm
#from Code.lib.FuseModule import GNNFuse
def maxpool():
...
class eca(nn.Module):
    def __init__(self, c):
        super().__init__()
        self.gba1 = eca_layer(c)
        self.gba2 = eca_layer(c)
        self.gba3 = eca_layer(c)
        self.gba4 = eca_layer(c)
        self.gba5 = eca_layer(c)

    def forward(self, feats):
        x1 = self.gba1(feats[0])
        x2 = self.gba2(feats[1])
        x3 = self.gba3(feats[2])
        x4 = self.gba4(feats[3])
        x5 = self.gba5(feats[4])
...
class GBARES(nn.Module):
    def __init__(self):
        super().__init__()
        self.rgb_encoder = Res2Net_model(50)
        self.depth_encoder = Res2Net_model(50)       #backbone
    ...
    def forward(self, x, d):
        ...
        rgb_feats = self.gba_model_x(rgb_feats)

        depth_feats = self.depth_encoder(d)
        depth_feats = self.gba_model_d(depth_feats)
...
class RGBDNet(nn.Module):
    def __init__(self, channel=32,ind=50):
        ...
        # funsion encoders #
        ###############################################
        self.fu_0 = FuseModule0(64, 64)#
        
        self.fu_1 = FuseModule1(256, 128) #MixedFusion_Block_IMfusion
        self.pool_fu_1 = maxpool()
        
        self.fu_2 = FuseModule1(512, 256)
        self.pool_fu_2 = maxpool()
        
        self.fu_3 = FuseModule1(1024, 512)
        self.pool_fu_3 = maxpool()

        self.fu_4 = FuseModule1(2048, 1024)
        ...
        self.ful_layer4   = GNNFuse(channel)
        self.ful_layer3   = GNNFuse(channel)
        self.ful_layer2   = GNNFuse(channel)
        self.ful_layer1   = GNNFuse(channel)
        self.ful_layer0   = GNNFuse(channel)
    ...
    def forward(self, imgs, depths):
        ...
        depths = self.layer_dep0(depths)
        rfeats, dfeats = self.encoder(imgs, depths)
